...
<div class="container pb-6 pt-6">
	...
	<h2 class="title has-text-centered"><?php echo $datos['producto_nom'];?></h2>
	...
	<form action="./php/producto_actualizar.php" method="POST" class="FormularioAjax" autocomplete="off" >
		...
		<div class="columns">
		  	...
		  	<div class="column">
		    	<div class="control">
					<label>Nombre</label>
				  	<input class="input" type="text" name="producto_nom" pattern="[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,$#\-\/ ]{1,70}" maxlength="70" required value="<?php echo $datos['producto_nom'];?>">
				</div>
		  	</div>
		</div>
		<div class="columns">
                      ...
                      <?php
                            ...
                            if($categorias->rowCount()>0){
                                ...
                                foreach($categorias as $rows){
                                    if($datos['categoria_id'] == $rows['categoria_id']){
                                        echo '<option value="'.$rows['categoria_id'].'" selected="" >'.$rows['categoria_nom'].' (Actual)</option>';
                                    }else{
                                        echo '<option value="'.$rows['categoria_id'].'" >'.$rows['categoria_nom'].'</option>';
                                    }
