<?php
	...
	if(isset($busqueda) && $busqueda!=""){
		...
		$consulta_datos="SELECT * FROM categoria WHERE categoria_nom LIKE '%$busqueda%' OR categoria_descripcion LIKE '%$busqueda%' ORDER BY categoria_nom ASC LIMIT $inicio,$registros";

		$consulta_total="SELECT COUNT(categoria_id) FROM categoria WHERE categoria_nom LIKE '%$busqueda%' OR categoria_descripcion LIKE '%$busqueda%'";

	}else{

		$consulta_datos="SELECT * FROM categoria ORDER BY categoria_nom ASC LIMIT $inicio,$registros";
	...
	if($total>=1 && $pagina<=$Npaginas){
		...
		foreach($datos as $rows){
			$tabla.='
				<tr class="has-text-centered" >
					<td>'.$contador.'</td>
                    <td>'.$rows['categoria_nom'].'</td>
                    <td>'.substr($rows['categoria_descripcion'],0,25).'</td>
                    <td>
